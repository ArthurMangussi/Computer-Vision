...
def dilation(image:cv2.imread, filter:np.ndarray)->np.ndarray:
    """
    Função de Morfologia Matemática: Dilatação

    Args:
        image (cv2.imread): A imagem que desejar dilatar
        filter (np.ndarray): O kernel para dilatação
    
    Return:
        output (np.ndarray): A imagem dilatada
    """
...
if __name__ == "__main__":
    ...
    img_erosion = dilation(image=image, filter=filter)
